...
fn main() -> Result<(), Box<dyn std::error::Error>>{
  ...
  let banca: String = prompt("Banca")?;
  let ano: u32 = prompt("Ano")?;
  let prova: String = prompt("Prova")?;
  ...
  loop{
    ...
    let texto_base_bak: String = match texto_base {
      ...
      None => {
        println!("Atenção, texto base vazio!");
        "".to_string()
      }
    };
    texto_base = match prompt_opt::<String, &str>("Texto base")? {
    ...
    // Questão
    let mut questao: String = prompt("Questão")?;
    ...
    match tipo.as_str() {
      "me" => {
        resposta_multipla = Some(prompt::<usize, &str>
          ("Entre o index da alternativa certa")?);
        match resposta_multipla {
          Some(num) if num > alternativas_length - 1 => {
            println!("
              Index além do no. de alternativas. Usando {}", 
              alternativas_length - 1)
          },
          _ => {} 
        }
      },
      _ => { 
        resposta_bool = match prompt::<String, &str>
           ("Verdadeiro ou falso?")?
           .as_str() {
    ...
    let imagem_bak: String = match imagem {
      ...
      None => {
        println!("Atenção, sem imagem!");
        "".to_string()
      }
    };
    imagem = match prompt_opt::<String, &str>("Imagem")? {
    ...
    let erro: Option<String> = prompt_opt(
            "Entre 'x' se houve erro para eliminar a questão")?;
    match erro {
      ...
      Some(_) | None => {
        ...
        println!("Entrada adicionada com sucesso!");
      },
    };

    // Entrar outra questão ou sair
    let continued: Option<String> = prompt_opt("Continuar?")?;
    match continued {
      Some(answer) if matches!(
        answer.as_str(), 
        "y" | "yes" | "sim" | "s" | "") => (),
